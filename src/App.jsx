...
const App = () => {
  ...
  const ai = new GoogleGenAI({
    apiKey: import.meta.env.VITE_GENAI_API_KEY // 🔒 secure usage
  });
  ...
  async function fixCode() {
    ...
    try {
      const result = await ai.models.generateContent({
        model: "gemini-2.0-flash",
        contents: `You're a senior developer. Fix or improve the following ${selectedOption.value} code. 
...
${code}`
      ...
      setCode(result.text); // Replace editor content
    } catch (error) {
      alert("Failed to fix the code. Please try again.");
      console.error(error);
    }
    setLoading(false);
  }
